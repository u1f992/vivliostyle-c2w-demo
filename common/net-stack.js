// c2w-net-proxy.wasm (ブラウザ内ネットワークスタック) を browser_wasi_shim で
// 実行する共通コア。ブラウザ (Web Worker) と Node.js (worker_threads) の両方から
// 使う。container2wasm の examples/wasi-browser/htdocs/stack-worker.js 由来。
//
// 前提グローバル: WASI (browser_wasi_shim)、Subscription/Event/EventType
// (wasi-util.js)、sock*/sendCert (worker-util.js)、postMessage
//
// 注意: examples 由来のコードは暗黙のグローバル代入 (for (i = 0; ...) など) を
// 含むため、このファイルを strict モードにしてはいけない。

// c2w-net-proxy を起動する。wasm: ArrayBuffer または Uint8Array
function startNetStack(wasm) {
    var fds = [
        undefined, // 0: stdin
        undefined, // 1: stdout
        undefined, // 2: stderr
