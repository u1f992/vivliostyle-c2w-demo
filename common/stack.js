    ...
    return function(msg){
        ...
        if (typeof req_ == "object" && req_.type) {
            ...
            case "http_send":
                ...
                if (reqObj.headers) {
                    // ゲスト由来のヘッダのうち CORS-safelisted なものだけを通す。
                    // Host や Connection など (ブラウザ管理) を fetch に渡しても
                    // 無視されるだけだが、safelisted でないヘッダが混ざると CORS
                    // プリフライトが発生し、対応しないサーバへの fetch が失敗する。
                    var allowedHeaders = ["accept", "accept-language", "content-language", "content-type"];
                    var filtered = {};
                    for (var k in reqObj.headers) {
                        if (allowedHeaders.includes(k.toLowerCase())) {
                            filtered[k] = reqObj.headers[k];
                        }
                    }
                    reqObj.headers = filtered;
                }
                var reqID = getID();
                if (reqID < 0) {
            ...
            case "http_writebody":
                ...
                if (req_.isEOF && !httpConnections[req_.id].requestSent) {
                    ...
                    fetch(connObj.address, connObj.request).then((resp) => {
                        // Headers オブジェクトは JSON.stringify で空 {} になるため
                        // 明示的に plain object へ変換する。また MITM プロキシとして
                        ...
                        console.error("[proxy-fetch-fail]", connObj.address, String(error));
                        connObj.response = new TextEncoder().encode(JSON.stringify({
                            status: 503,
