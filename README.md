...
| `index.html`／`main.js` | UI（textarea＋Buildボタン）。PDF完成時に自動ダウンロード |
| `demo-worker.js` | VM実行worker。WASI shimへのパッチとargv/envの組み立て |
...
| `stack.js`／`stack-worker.js`／`worker-util.js`／`wasi-util.js` | ネットワークスタックの結線（container2wasm examples由来、修正あり） |
| `coi-serviceworker.js` | cross-origin isolationの付与 |
| `test/run-test.mjs` | E2Eテスト |
...
ネットワーク経路の詳細（Node 24の`NODE_USE_ENV_PROXY`によるゲスト内Nodeのプロキシ対応、レスポンスヘッダ転送、CORS-safelistedヘッダのフィルタリングなど）はソースコメントを参照。
