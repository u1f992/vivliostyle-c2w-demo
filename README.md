...
### ブラウザ実行時に残るコンソールエラー（ゲスト由来）

ビルド中、DevToolsに次のエラーが出るが、いずれもゲストVM内のChromiumが自身のテレメトリへ接続しようとして失敗するもので、組版には影響しない（同一セッションでWebフォントの取得とPDF生成は成功する）。c2w-net-proxyはゲストのHTTP要求をページ文脈の`fetch()`（`common/stack.js`）に変換するため、デモページのセキュリティポリシーがそのまま適用されることが原因で、ゲストが直接ネットワークへ出るネイティブの`docker run`では発生しない。

| コンソール表示 | 発生源 | ブロック理由 |
|---|---|---|
| `Mixed Content: ... 'http://clients2.google.com/time/...'` | component updaterの時刻取得（平文HTTP） | HTTPSページからの`http://`取得 |
| `blocked by CORS policy`＋`[proxy-fetch-fail]`（`accounts.google.com/ListAccounts`／`android.clients.google.com/checkin`／`www.google.com/async/folae`／`content-autofill.googleapis.com`） | アカウント同期・デバイスcheckin・autofill等のテレメトリ | エンドポイントが`Access-Control-Allow-Origin`を返さない |

失敗したリクエストはゲストへ503として返り、ゲスト側はテレメトリの失敗として処理する。発生源ごと止めるには、`Dockerfile.rootwrap`のChromium起動引数に`--disable-background-networking`等を追加してイメージを変換し直す（デプロイworkflowの通常経路で再変換される）。
